/**
	...
	we prevent deadlock by adding a global variable "turn" that represent Queues every phil has number from 0 to 4
	every loop we letting only the first phil to eat then increment the varibable by one % N(5) by doing that we
	controling the pace and who eat when.. by doing it only one phil eat at a time and it will prevent deadlock
